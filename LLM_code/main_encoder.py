...
from model_utils.projector import EncoderProjectorConcat, EncoderProjectorQFormer
from model_utils.model import speechLLM
...
parser.add_argument(
    "--projector",
    default='linear',
    help='projector: linear, q-former'
)
...
parser.add_argument(
    "--downsample_rate",
    default='5',
    help='downsample rate for audio encoder output'
)
...
def _get_input_dict(batch):
    input_ids, labels, attention_mask, type_token_ids, audio_features, audio_masks = batch["input_ids"], \
        batch["labels"], batch["attention_mask"], batch["type_token_ids"], batch["audio_features"], batch["audio_masks"]
...
if "chatglm" in args.model_name_or_path:
    ...
    if "bloom" in args.model_name_or_path or "falcon" in args.model_name_or_path:
        ...
        config = AutoConfig.from_pretrained(args.model_name_or_path)
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
        llm =AutoModelForCausalLM.from_pretrained(args.model_name_or_path).half()
...
if args.checkpoint_dir is not None:
    ...
    llm = load_state_dict_from_zero_checkpoint(llm, args.checkpoint_dir)


num_parameters = get_parameter_number(llm)
...
if args.feature != 'simple':
    if args.encoder == 'hubert':
        encoder = AutoModel.from_pretrained('hubert-base-ls960')
    elif args.encoder == 'w2v':
        encoder = AutoModel.from_pretrained('wav2vec2-base-960h')
    elif args.encoder == 'd2v':
        encoder = AutoModel.from_pretrained('facebook/data2vec-audio-base') # facebook/data2vec-audio-large facebook/data2vec-audio-base-960h
        args.encoder_size = 'small'
    elif args.encoder == 'whisper':
        encoder = AutoModel.from_pretrained('openai/whisper-small')

    if args.projector == 'linear':
        encoder_projector = EncoderProjectorConcat(args)
    elif args.projector == 'q-former':
        encoder_projector = EncoderProjectorQFormer(args)
    ...
    if args.freeze_llm=='True':
        for param in llm.parameters():
            param.requires_grad = False
    model = speechLLM(encoder, llm, encoder_projector, args)
    # # freeze encoder
    # if args.freeze_encoder=='True':
    ...
    model = speechLLM(None, llm, None, args)
...
# model.eval()
# inputs = _get_input_dict(next(iter(train_dataloader)))
# inputs = {k: v.to(torch.bfloat16) if isinstance(v, torch.Tensor) and torch.is_floating_point(v)  else v for k, v in inputs.items()}
# out = model(**inputs)
...
if __name__ == "__main__":
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(batch_iterator):
                if args.emotion_prediction:
                    ...
                    batch = _get_input_dict(batch)
                    batch = {k: v.to(torch.bfloat16) if isinstance(v, torch.Tensor) and torch.is_floating_point(v) else v for k, v in batch.items()}
                    outputs = model(**batch)
                    loss = outputs.loss
            ...
            for eval_step, eval_batch in enumerate(tqdm(eval_dataloader)):
                ...
                eval_batch = {k: v.to(torch.bfloat16) if isinstance(v, torch.Tensor) and torch.is_floating_point(v) else v for k, v in eval_batch.items()}
                with torch.no_grad():
                    if "chatglm" in args.model_name_or_path:
                        ...
                        outputs = model.generate(**eval_batch)
            ...
            for index, o in enumerate(outs):
                ...
                #     else:
                #         print(o)
                #         output_ids = all_outputs[index][59+args.max_seq_length: ]
                #         answer1 = tokenizer.decode(output_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                #         print(answer1)
                #         print('\n')
                # else:
                answer = o
    ...
    if not args.do_train and args.do_eval:
        ...
        for eval_step, eval_batch in enumerate(tqdm(eval_dataloader)):
            ...
            eval_inputs_iter.extend(eval_batch["input_ids"])
            max_length_this_batch = eval_batch["input_ids"].size(-1) if args.model_type == "decoder" else 0
            eval_batch = {k: v.to(torch.half) if isinstance(v, torch.Tensor) and torch.is_floating_point(v) else v for k, v in eval_batch.items()}
            with torch.no_grad():
                if "chatglm" in args.model_name_or_path:
                    outputs = model.generate(
                        **eval_batch,
